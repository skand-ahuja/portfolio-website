...
type RoleType = { id: string; role: string; company: string; duration: string; current: boolean; description: string; skills: string[] };

function ExperienceItem({ item, index, isLast }: { item: RoleType; index: number; isLast: boolean }) {
  return (
    <motion.article {...fadeUp(index * 0.1)} className="relative grid grid-cols-[28px_1fr] gap-4 md:grid-cols-[160px_32px_1fr] md:gap-6">
      <div className="hidden pt-1 md:block">
        <div className="flex items-center gap-2 text-[var(--text-muted)]">
          <FontAwesomeIcon icon={faCalendarDays} className="h-3 w-3 text-[var(--accent)]" />
          <span className="font-mono text-[10px] font-medium uppercase tracking-wider">{item.duration}</span>
        </div>
      </div>
      <div className="relative flex justify-center">
        ...
        <span className="relative z-10 mt-1.5 flex h-3.5 w-3.5 items-center justify-center rounded-full border-[1.5px] border-[var(--accent)] bg-[var(--page-bg)]">
      ...
      <div className={isLast ? "pb-0" : "pb-12 md:pb-16"}>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {item.skills.map((skill) => (
            <span key={skill} className="rounded-md border border-[var(--border)] bg-[color-mix(in_srgb,var(--surface-solid)_40%,transparent)] px-2.5 py-1 font-mono text-[9px] uppercase tracking-wider text-[var(--text-muted)] transition-colors hover:border-[color-mix(in_srgb,var(--accent)_30%,transparent)] hover:text-[var(--text-primary)]">
              {skill}
            </span>
          ))}
        </div>
        ...
        <motion.div {...fadeUp(0)} className="mt-14 md:ml-[218px]">
          <div className="glass-card flex flex-col items-center justify-between gap-4 rounded-2xl p-5 sm:flex-row">
            <div className="flex items-center gap-4">
              <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-[color-mix(in_srgb,var(--accent)_10%,transparent)] text-[var(--accent)]">
