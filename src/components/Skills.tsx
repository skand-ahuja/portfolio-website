...
type CategoryType = { id: string; number: string; label: string; title: string; description: string; icon: IconDefinition; skills: SkillType[] };

const SKILL_CATEGORIES: CategoryType[] = [
...
function SkillLogo({ skill, isActive, isHovered }: { skill: SkillType; isActive: boolean; isHovered: boolean }) {
...
function SkillItem({ skill, isActive, onToggle }: { skill: SkillType; isActive: boolean; onToggle: () => void }) {
...
function SkillCategory({ category, categoryIndex }: { category: CategoryType; categoryIndex: number }) {
  const [activeSkill, setActiveSkill] = useState<string | null>(null);
  const handleToggle = (name: string) => setActiveSkill(cur => cur === name ? null : name);
    ...
    <motion.article {...fadeUp(categoryIndex * 0.07)} className={`grid grid-cols-1 gap-7 pb-12 md:grid-cols-[0.34fr_0.66fr] md:gap-12 lg:grid-cols-[0.3fr_0.7fr] lg:gap-16 ${categoryIndex !== SKILL_CATEGORIES.length - 1 ? "border-b border-[var(--border)]" : ""}`}>
      <div>
        <div className="mb-4 flex items-center gap-3">
          <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-[color-mix(in_srgb,var(--accent)_8%,transparent)] text-[var(--accent)]">
            <FontAwesomeIcon icon={category.icon} className="h-3.5 w-3.5" />
          </span>
          <span className="font-mono text-[10px] font-semibold uppercase tracking-widest text-[var(--accent)]">{category.number} &middot; {category.label}</span>
        </div>
