...
function JourneyItem({ stage, index, isLast }: { stage: JourneyType; index: number; isLast: boolean }) {
    return (
        <motion.article
        ...
        >
            <div className="relative flex justify-center">
                ...
                {/* Solid background blocks the line, inner span provides the tint */}
            ...
            <div className={isLast ? "pb-0" : "pb-8"}>
                <div className="mb-1.5 flex flex-wrap items-center gap-2">
                    <span className="font-mono text-[9px] font-semibold tracking-widest text-[var(--accent)]">{stage.number}</span>
                    <span className="h-px w-3 bg-[var(--border)]" />
                    <span className="font-mono text-[9px] tracking-widest text-[var(--text-muted)]">{stage.period}</span>
                </div>
                ...
                <div className="mt-3 flex flex-wrap gap-x-2 gap-y-1">
                    {stage.skills.map((skill, i) => (
                        <span key={skill} className="flex items-center gap-2 text-[11px] text-[var(--text-secondary)]">
                            {skill}
                            {i < stage.skills.length - 1 && <span className="h-1 w-1 rounded-full bg-[var(--accent)] opacity-50" />}
                        </span>
                    ))}
                </div>
            </div>
        ...
        <section id="about" className="relative px-6 py-20 md:px-12 md:py-24">
            <div className="mx-auto max-w-6xl">
                ...
                >
                    ...
                    <span className="font-mono text-[10px] font-semibold uppercase tracking-widest text-[var(--accent)]">&gt; ORIGIN_STORY.MD</span>
                </motion.div>
                ...
                <div className="grid grid-cols-1 gap-14 lg:grid-cols-[1.05fr_0.95fr] lg:gap-20 xl:gap-24">
                    <div>
                        <motion.h2
                            ...
                            whileInView={{ opacity: 1, y: 0 }}
                            viewport={{ once: true, margin: "-60px" }}
                            className="max-w-xl text-4xl font-bold tracking-tight text-[var(--text-primary)] lg:text-5xl"
                        >
                            From wiring circuits to wiring <span className="text-[var(--accent)]">systems.</span>
                        </motion.h2>

                        <div className="mt-6 max-w-xl space-y-4">
                            {shortNarrative.map((p, i) => (
                                <motion.p
                                    key={i}
                                    ...
                                    className="text-[14.5px] leading-relaxed text-[var(--text-secondary)]"
                                >
                                    {p}
                                </motion.p>
                            ))}
                        ...
                        <motion.div
                            ...
                            whileInView={{ opacity: 1, y: 0 }}
                            viewport={{ once: true }}
                            className="group relative mt-7 max-w-xl border-l-2 border-[color-mix(in_srgb,var(--accent)_40%,transparent)] pl-5 transition-colors hover:border-[var(--accent)]"
                        >
                            ...
                            <p className="text-[14.5px] leading-relaxed text-[var(--text-secondary)]">
                                <span className="font-semibold text-[var(--text-primary)]">{aboutHighlight}</span> {aboutClosing}
                            </p>
                        </motion.div>
                    </div>
                    ...
                    <motion.div
                        ...
                        className="lg:pt-1"
                    >
                        <div className="mb-7 flex items-center justify-between border-b border-[var(--border)] pb-3">
                            <span className="font-mono text-[9px] font-semibold uppercase tracking-widest text-[var(--text-muted)]">Journey</span>
                            <span className="hidden font-mono text-[9px] uppercase tracking-widest text-[var(--text-muted)] sm:block">Eng &rarr; Data &rarr; Code</span>
                        </div>
                        <div>
                            {JOURNEY.map((stage, i) => (
                                <JourneyItem key={stage.id} stage={stage} index={i} isLast={i === JOURNEY.length - 1} />
                            ))}
                        </div>
                    </motion.div>
                </div>
