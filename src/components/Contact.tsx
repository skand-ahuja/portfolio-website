...
function validate(form: typeof INITIAL_FORM) {
    const errors: Partial<Record<keyof typeof INITIAL_FORM, string>> = {};
...
// 🍏 RESTORED ORIGINAL KEYBOARD-ACCESSIBLE LISTBOX DROPDOWN
function InquiryDropdown({ value, onChange, error }: { value: string; onChange: (e: { target: { name: string; value: string } }) => void; error?: string }) {
    ...
    function chooseOption(option: typeof INQUIRY_TYPES[0]) {
            ...
            >
                {selectedOption ? (
                    <span className="flex min-w-0 items-center gap-2.5">
                        <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-[color-mix(in_srgb,var(--accent)_10%,transparent)] text-[var(--accent)]">
                    ...
                    >
                        {INQUIRY_TYPES.map((option, index) => {
                            ...
                            return (
                                ...
                                >
                                    <span
                                        className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${isSelected ? "bg-[var(--accent)] text-white" : "bg-[color-mix(in_srgb,var(--accent)_10%,transparent)] text-[var(--accent)]"
                                            }`}
                                    >
...
// 🍏 RESTORED MICRO-INTERACTION SOCIAL LINKS
function SocialLink({ social }: { social: typeof SOCIAL_LINKS[0] }) {
    ...
    return (
        <a
            ...
            style={{
                borderColor: hovered ? social.hoverColor : "var(--border)",
        ...
        >
            ...
            <FontAwesomeIcon
                ...
                style={{ opacity: hovered ? 1 : 0.4, transform: hovered ? "translate(2px, -2px)" : "translate(0, 0)" }}
            />
        </a>
    );
}
...
export default function Contact() {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement> | { target: { name: string; value: string } }) => {
        ...
        if (errors[name as keyof typeof INITIAL_FORM]) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // In Next.js Phase 8, this calls our native Next.js API Route `/api/contact`
            const apiUrl = process.env.NEXT_PUBLIC_API_URL ? `${process.env.NEXT_PUBLIC_API_URL}/api/contact` : "/api/contact";
            const response = await fetch(apiUrl, {
                ...
                body: JSON.stringify({
                    ...
                    website: form.website // Honeypot field for bot protection
                })
            });
            ...
            // Graceful fallback for now until Phase 8 backend is linked
            setToast({ message: "Mock Mode: Phase 8 backend will handle this!", type: "success" });
        } finally {
        ...
        <section id="contact" className="relative overflow-hidden px-6 py-20 md:px-12 md:py-24">
            <div className="relative z-10 mx-auto max-w-6xl">
                <div className="grid grid-cols-1 gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:gap-20">
                    ...
                    <motion.div initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-70px" }} transition={{ duration: 0.5 }}>
                        ...
                        <div className="mt-6 flex items-center gap-3">
                            <span className="relative flex h-2 w-2">
                                <span className="absolute h-full w-full animate-ping rounded-full bg-[var(--color-success)] opacity-75" />
                        ...
                        <div className="mt-8 grid grid-cols-2 gap-6 border-t border-[var(--border)] pt-6">
                            <div>
                                <div className="mb-1.5 flex items-center gap-2 text-[var(--accent)]">
                                    <FontAwesomeIcon icon={faLocationDot} className="h-3 w-3" />
                                    <span className="font-mono text-[9px] uppercase tracking-widest">Location</span>
                                </div>
                            ...
                            <div>
                                <div className="mb-1.5 flex items-center gap-2 text-[var(--accent)]">
                                    ...
                                    <span className="font-mono text-[9px] uppercase tracking-widest">Response</span>
                                </div>
                                <p className="text-[13px] font-medium text-[var(--text-primary)]">{SECTION_CONFIG.response}</p>
                            </div>
                        </div>

                        {/* RESTORED ELSEWHERE SOCIAL PILLS */}
                        <div className="pt-8">
                    ...
                    {/* RIGHT COLUMN: COMPLETE FORM WITH GLASS AESTHETIC */}
                    <motion.div
                    ...
                    >
                        ...
                        <form onSubmit={handleSubmit} noValidate className="space-y-5">
                            {/* Bot Honeypot */}
                            <input type="text" name="website" value={form.website} onChange={handleChange} style={{ position: "absolute", left: "-9999px", opacity: 0 }} tabIndex={-1} autoComplete="off" />
                            ...
                            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                                ...
                                <div>
                                    <label className="mb-1.5 block font-mono text-[10px] font-semibold uppercase tracking-widest text-[var(--text-secondary)]">
                                        Company <span className="font-normal normal-case tracking-normal text-[var(--text-muted)]">(optional)</span>
                                    </label>
                            ...
                            <div>
                                <div className="mb-1.5 flex items-center justify-between gap-3">
                                    ...
                                    <span className="font-mono text-[9px]" style={{ color: form.message.length > 900 ? "var(--color-error)" : "var(--text-muted)" }}>
                                ...
                                <textarea
                                    ...
                                    rows={4}
                                    className={`form-control min-h-[120px] resize-y ${errors.message ? "form-control-error" : ""}`}
                            ...
                            <button type="submit" disabled={isSubmitting} className="btn-primary w-full">
                                {isSubmitting ? (
                                    <>
                                        <FontAwesomeIcon icon={faSpinner} spin className="h-4 w-4" /> Sending...
                                    </>
                                ) : (
                                    <>
                                        Send Message <FontAwesomeIcon icon={faArrowRight} className="h-3.5 w-3.5" />
                                    </>
                                )}
                            </button>
